...
const BannerSection = ({ category }) => {
  ...
  return (
    <div className="relative sm:h-[420px] h-[80vh] w-full flex flex-col items-center justify-center text-white text-center px-6 overflow-hidden bg-gradient-to-r from-[#1B2E36] via-[#1B2E36] to-[#1B2E36]">
      {/* Decorative Ribbons */}
      ...
      >
        {category.code === "Click below" ? (
          ...
          <>
            ...
            <span className="text-[#0faf59]">{category.code}</span>
          </>
        ) : (
          <>Binance Red Packet</>
        )}
      ...
      <a
        ...
        target="_blank"
        className={`mt-6 px-6 py-3 text-xl font-semibold bg-[#0faf59] rounded-full transition-all duration-200 delay-300 hover:opacity-50 flex items-center gap-2 ${
          isVisible ? "translate-y-0 opacity-100" : "translate-y-10 opacity-0"
        }`}
      >
        {category.name !== "Binance red packet" ? (
