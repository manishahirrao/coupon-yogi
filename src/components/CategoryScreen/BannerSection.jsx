...
import coupon from "./coupon2.png";
const BannerSection = ({ category }) => {
  ...
  function getDailyOffer() {
    ...
    const result = Math.floor((random / 233280) * (100 - 40 + 1)) + 40;
    return `$${result}`;
  }
    ...
    >
      ...
      {category.name === "Binance red packet" && (
        <div className="flex items-center justify-center flex-col">
          <p
            className={`text-lg font-semibold max-w-2xl transition-all duration-700 delay-200 ${
              isVisible
          ...
          <div className="relative w-46 h-38 mb-4">
            ...
            >
              {/* Image (coupon) positioned at top */}
              <img
                className="absolute top-28  left-1/2 transform -translate-x-1/2 w-20 h-14"
                src={coupon}
                alt="Coupon"
              />

              {/* Text Overlay */}
              <p className="absolute inset-0 top-24 flex items-center justify-center text-white font-bold text-[2rem]">
                {getDailyOffer()}
              </p>
            </div>
          </div>
